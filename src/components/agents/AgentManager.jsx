...
export function AgentManager() {
  ...
  const selectedAgentTeam = teams.find(t => t.id === selectedAgent?.teamId);
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: 24 }}>
      ...
      <div className="glass-panel" style={{ padding: 20, height: 'fit-content' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <h3 style={{ fontSize: 16, fontWeight: 800, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <Users size={18} color="var(--pioneers-cyan)" /> Temsilciler ({agents.length})
          </h3>
          <button
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
          ...
          >
            <option value="all">Tüm Takımlar ({agents.length})</option>
            {teams.map(t => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 600, overflowY: 'auto' }}>
          {filteredAgents.length === 0 ? (
            <div style={{ padding: 30, textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
              Kayıtlı temsilci bulunamadı. "Yeni Temsilci" butonuna basarak ilk çalışanı ekleyebilirsiniz.
            </div>
          ) : (
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{ background: ag.avatarBg || agTeam?.color || '#3b82f6' }}
                  >
                    {ag.avatar || 'AG'}
                  </div>
      ...
      {/* Right Column: Selected Agent Profile, Credentials & Rules */}
      {selectedAgent ? (
        ...
        <div className="glass-panel" style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>
          Lütfen sol taraftan bir temsilci seçin veya yeni temsilci ekleyin.
        </div>
      )}
